...
The MCP Servers pane lives in the **Plugins** section of the Settings sidebar.

1. Open **Settings** — click the EnConvo icon in the macOS menu bar and choose **Settings...**, or press `Cmd + ,`
2. In the left sidebar, scroll down to the **Plugins** section and click **MCP Servers**
3. Click **Add server**
4. Choose a transport type (stdio, HTTP, or SSE) and fill in the configuration
5. Toggle the switch to enable/disable

<Tip>
The Settings sidebar is long — press `Cmd + F` inside Settings and type "MCP" to filter the sidebar and jump straight to the pane. You can also open it directly with the deeplink `enconvo://preference?pane=mcp_management`.
</Tip>
...
1. In **Settings** → **Plugins** → **MCP Servers**, click **Import**
