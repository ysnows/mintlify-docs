...
2. Find the **Computer Use** extension
3. Set **Default Browser** to your preferred browser

### Verify Connection

To check if the extension is connected, use the `computer-use/browser/status` API route:
...
<AccordionGroup>
  ...
  <Accordion title="Wrong browser is being controlled">
    ...
    ```
    computer-use/browser/navigate { "url": "https://example.com", "browser": "edge" }
    ```
