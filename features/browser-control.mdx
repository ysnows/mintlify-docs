...
<Tip>
Use Browser Control for websites and browser tabs. Use
[Computer Use](/features/computer-use) for native macOS apps, file pickers,
system dialogs, and desktop app workflows.
</Tip>
...
- **Fresh page snapshots** are taken after navigation, loading, clicking, and
  DOM changes so element references stay current.
- **Cross-origin iframe handling** improves interaction with embedded forms and
  dashboards.
- **Drag-safe clicks and coordinate actions** help with canvases, custom UI
  widgets, and controls that do not expose clean accessibility labels.
- **Clipboard proxy support** preserves formatted text when pasting into rich
  editors.
- **Cleaner page-state output** makes it easier for agents to choose the right
  link, button, textbox, or table row.

If a task still fails, ask EnConvo to take a new snapshot or describe the
visible page before trying the next action.
