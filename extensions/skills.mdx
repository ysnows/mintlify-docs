...
<CardGroup cols={2}>
  <Card title="Explicit Invocation" icon="hand-pointer">
    Mention the skill directly in your prompt using `/skills` or the `$` prefix. Use this when you know exactly which skill you need.
  </Card>
...
Disable a skill without deleting it by updating your configuration:
...
Restart EnConvo after changing the configuration.

### Invocation Policy

Control whether a skill can be implicitly invoked by adding metadata:

```yaml
# agents/enconvo.yaml
policy:
  allow_implicit_invocation: false  # Only explicit $skill invocation works
```

### UI Metadata

Customize how the skill appears in EnConvo:
...
Browse and install skills shared by the community:

```
$skill-installer install the translation skill
```

You can also install from a GitHub repository:

```
$skill-installer install https://github.com/user/enconvo-skills
```

EnConvo detects newly installed skills automatically.
