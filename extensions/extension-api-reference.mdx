...
description: "SDK patterns and API entry points for developing Enconvo extensions"
---
...
Registers the main entry point handler for a command. The build/runtime adapter uses this registration mechanism. For ordinary plugin command files, use the default-exported handler shown in [Developing Extensions](/extensions/developing), rather than registering the same handler twice.
...
Send a request to the native macOS app. Returns a Promise for the matching response. Use `sendAsync` only when the operation does not need a result.
...
environment.sessionId;           // Current session ID
...
return EnconvoResponse.messages(
  ...
  [
    ...
    Action.Retry({ sessionId: environment.sessionId })
  ]
);
```
...
npx enconvo plugin build

# Dev mode with hot reload
npx enconvo plugin build --dev

# Generate API schema
npx enconvo plugin generate-schema

# Publish to store
npx enconvo plugin publish
```
