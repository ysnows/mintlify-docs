...
When you invoke EnConvo (via SmartBar, Dynamic Island, App Sidebar, or keyboard shortcut), the system collects context through this pipeline:
...
3. **Active application context** -- when enabled and available
...
Captured context can become part of a saved conversation or knowledge base item. Model requests and selected OCR, embedding, transcription, or other tool providers may receive relevant content. Review [Privacy & Security](/configuration/privacy-security) for the distinction between local storage and provider processing.

A local chat model alone does not make every tool offline. Check the whole flow, including browser access, captures, and provider-based processing. App exclusions and macOS permissions reduce available context; they are not a promise that every third-party app field can be classified as sensitive.
