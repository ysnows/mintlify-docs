...
### Linked Folders (Folder Sync)

Import a folder from disk — for example an Obsidian vault — and keep it in
sync with your knowledge base:

1. Open your knowledge base and click **Add → Folder** (or **Import Folder**
   when the knowledge base is empty).
2. Review the import preview — file counts by type and anything that will be
   skipped — then confirm.
3. The folder's structure is mirrored into the knowledge base's folder tree.
   Documents are indexed first so text search works right away; images (OCR)
   and audio/video (transcription) follow.
4. Select the folder and turn on **Auto-sync** to watch it while EnConvo runs
   — changes sync shortly after you stop editing a file, and the folder is
   also reconciled at every launch. Or click **Sync Now** anytime.

How syncing behaves:

- The folder on disk is the source of truth: new files are added, edited
  files are re-indexed (only when their content actually changed), renamed or
  moved files are tracked without re-indexing, and files deleted on disk are
  removed from the knowledge base.
- Hidden files and folders (like `.git` and `.obsidian`), unsupported types,
  and files over 50 MB are skipped — the folder's details panel shows a tally
  of what was skipped.
- EnConvo never modifies or deletes the files on disk.
- If the source folder can't be found (for example, an unplugged external
  drive), nothing is removed from the knowledge base — the folder is marked
  and you can relink it to its new location.
