...
Both short event names and full internal names work as keys inside `hooks`. For
example, `run_status_changed` and `event-run_status_changed` match the same event.
The old flat JSON shape is still readable for existing installs, but new writes
use the wrapped format.
