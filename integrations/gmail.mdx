...
EnConvo uses Google OAuth to securely connect to your Gmail account. Sign-in uses Google authorization; review the requested access before granting it.
...
<Steps>
  ...
  <Step title="Choose Credential Provider">
    Select either **Gmail (Enconvo Cloud)** (recommended, pre-configured OAuth) or your own **Gmail** credentials
  </Step>
  ...
  <Step title="Start Using Gmail Tools">
    Enable the Gmail tools for the Agent you intend to use, then test a read-only request in that Agent's conversation
  </Step>
</Steps>

<Tip>
The **Gmail (Enconvo Cloud)** credential option provides a pre-configured OAuth setup so you can get started without creating your own Google API project.
</Tip>
